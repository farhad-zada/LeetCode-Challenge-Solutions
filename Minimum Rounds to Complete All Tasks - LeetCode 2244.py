...
Input: tasks = [2,3,3]
Output: -1
Explanation: There is only 1 task of difficulty level 2, but in each round, you can only 
complete either 2 or 3 tasks of the same difficulty level. Hence, you cannot complete all 
the tasks, and the answer is -1.
...
class Solution:
    def minimumRounds(self, tasks: List[int]) -> int:
        ...
        rounds = 0
        for k in Counter(tasks).keys():
            if counter[k] == 1: return -1
            rounds += 1 if counter[k] % 3 else 0 
            rounds += counter[k] // 3
        return rounds
...
# For any kind of discussion you can get in touch with me via:
