...
sold at the supermarket are represented as two parallel integer arrays products 
and prices, where the ith product has an ID of products[i] and a price of prices[i].
...
bill with the discount applied (if any). Answers within 10-5 of the actual value will be accepted.
...
class Cashier:
    ...
    def __init__(self, n: int, discount: int, products: List[int], prices: List[int]):
        ...
        self.pro = {p:prices[i] for i, p in enumerate(products)}
        self.t = 0
        

    def getBill(self, product: List[int], amount: List[int]) -> float:
        self.t += 1
        bill = sum([self.pro[p]*amount[i] for i, p in enumerate(product)])
        if self.t % self.n == 0: return bill*self.dis
        return bill
